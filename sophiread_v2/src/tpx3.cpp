...
std::vector<Hit> readTimepix3RawData(const std::string& filepath) {
    ...
    // (implementation of this will depend on the specific format of the TimePix3 data)
    std::vector<Hit> hits;
    ...
    for (auto i = buffer.begin(); i != buffer.end(); i+=8) {
        ...
        if (char_array[0]=='T' && char_array[1] == 'P' && char_array[2] == 'X') {
            ...
            // NOTE:
            // Sophiread_v1 is using a method that leads to negative data_packet_size and data_packet_num,
            // therefore we are using the code from manufacture example, tpx3cam.cpp to get the
            // data_packet_size.
            ...
            for (auto j = 0; j < data_packet_num; j++){
                ...
                if (data_packet[7] == 0x6F) {
                    ...
                    // timing section
                    spider_time = (unsigned short *)(&data_packet[0]);     // Spider time  (16 bits)
                    ...
                    // position section
                    npixaddr = (unsigned int *)(&data_packet[4]);          // Pixel address (14 bits)

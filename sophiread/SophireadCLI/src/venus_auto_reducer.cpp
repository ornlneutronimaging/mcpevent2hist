...
void print_usage(const char* program_name) {
  spdlog::info(
      "Usage: {} -i <input_dir> -o <output_dir> [-u <user_config_json>] [-f <tiff_file_name_base>] [-m <tof_mode>] [-c "
      "<check_interval>] [-v] [-d]",
      program_name);
  spdlog::info("Options:");
 ...
 */
void process_existing_files(const std::string& input_dir, const std::string& output_dir, const std::string& tiff_base,
  ...
  for (const auto& entry : fs::directory_iterator(input_dir)) {
    if (entry.is_regular_file() && entry.path().extension() == ".tpx3") {
      ...
      try {
        ...
        // Create TOF images
        auto tof_images =
            sophiread::timedCreateTOFImages(batches, config.getSuperResolution(), config.getTOFBinEdges(), tof_mode);
...
void monitor_directory(const std::string& input_dir, const std::string& output_dir, const std::string& tiff_base,
                       const std::string& tof_mode, const IConfig& config,
                       std::unordered_set<std::string>& processed_files, int check_interval) {
  ...
  while (true) {
    ...
    for (const auto& entry : fs::directory_iterator(input_dir)) {
      if (entry.is_regular_file() && entry.path().extension() == ".h5" && entry.path().stem().extension() == ".nxs") {
...
int main(int argc, char* argv[]) {
  try {
    ...
    std::unordered_set<std::string> processed_files;
    monitor_directory(options.input_dir, options.output_dir, options.tiff_base, options.tof_mode, *config,
                      processed_files, options.check_interval);

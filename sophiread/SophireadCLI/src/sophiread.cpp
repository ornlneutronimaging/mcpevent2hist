...
void print_usage(const char* program_name) {
  spdlog::info(
      "Usage: {} -i <input_tpx3> -H <output_hits> -E <output_events> [-u <config_file>] [-T <tof_imaging_folder>] [-f "
      "<tof_filename_base>] [-m <tof_mode>] [-d] [-v]",
      program_name);
  spdlog::info("Options:");
...
int main(int argc, char* argv[]) {
  try {
    ...
    if (!options.output_tof_imaging.empty()) {
      auto tof_images = sophiread::timedCreateTOFImages(batches, config->getSuperResolution(), config->getTOFBinEdges(),
                                                        options.tof_mode);
      sophiread::timedSaveTOFImagingToTIFF(options.output_tof_imaging, tof_images, config->getTOFBinEdges(),

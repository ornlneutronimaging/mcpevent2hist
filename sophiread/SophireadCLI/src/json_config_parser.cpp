 ...
 */
JSONConfigParser::JSONConfigParser(const nlohmann::json& config) : m_config(config) { parseTOFBinning(); }

/**
 * @brief Get the ABS Radius
 * @return double
 */
double JSONConfigParser::getABSRadius() const { return m_config.value("/abs/radius"_json_pointer, DEFAULT_ABS_RADIUS); }
 ...
 */
std::vector<double> JSONConfigParser::getTOFBinEdges() const { return m_tof_binning.getBinEdges(); }
...
std::string JSONConfigParser::toString() const {
  std::stringstream ss;
  ss << "ABS: radius=" << getABSRadius() << ", min_cluster_size=" << getABSMinClusterSize()

 ...
 *
 * This function creates time-of-flight (TOF) images based on the provided batches of TPX3 data. The TOF images are 2D
 * histograms that represent the distribution of neutron events in space for different TOF bins. The function takes into
 * account the super resolution, TOF bin edges, and the mode of operation (hit or neutron) to generate the TOF images.
 *
 * @param[in] batches The vector of TPX3 batches containing the neutron events.
 * @param[in] super_resolution The super resolution factor used to calculate the dimensions of each 2D histogram.
 * @param[in] tof_bin_edges The vector of TOF bin edges used to determine the TOF bin for each neutron event.
 * @param[in] mode The mode of operation, either "hit" or "neutron", which determines the type of events to process.
 * @return std::vector<std::vector<std::vector<unsigned int>>> The vector of TOF images, where each TOF image is a 2D
 * histogram representing the distribution of neutron events in space for a specific TOF bin.
 */
      ...
      if (std::filesystem::exists(filename)) {
        ...
        if (existing_tif) {
          ...
          if (existing_width == width && existing_height == height) {
            ...
            spdlog::error("Dimension mismatch for file: {}. Expected {}x{}, got {}x{}. Overwriting.", filename, width,
                          height, existing_width, existing_height);
          }

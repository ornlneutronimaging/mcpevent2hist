...
class UserConfig : public IConfig {
 public:
  UserConfig();
  UserConfig(double abs_radius, unsigned long int abs_min_cluster_size, unsigned long int abs_spider_time_range);

  double getABSRadius() const override { return m_abs_radius; }
  void setABSRadius(double abs_radius) { m_abs_radius = abs_radius; }

  unsigned long int getABSMinClusterSize() const override { return m_abs_min_cluster_size; }

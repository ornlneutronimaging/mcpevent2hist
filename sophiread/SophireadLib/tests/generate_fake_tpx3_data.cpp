...
unsigned long long packDataHeader(unsigned long long t, unsigned long long p,
                                  unsigned long long x,
                                  unsigned long long three,
                                  unsigned long long chip_nr,
  ...
  header = ((num_bytes & 0x000000000000FFFF) << 48) |
           ...
           ((x & 0x00000000000000FF) << 16) | ((p & 0x00000000000000FF) << 8) |
...
unsigned long long packPixelHit(unsigned long long spider_time,
                                unsigned long long ftoa, unsigned long long TOT,
                                unsigned long long TOA,
                                unsigned long long pixaddr) {
  unsigned long long temp;
  unsigned long long header = 0xb;

  temp =
      ((header & 0x00000000000000FF) << 60) |
      ((pixaddr & 0x000000000000FFFF) << 44) |
      ((TOA & 0x0000000000003FFF) << 30) | ((TOT & 0x00000000000003FF) << 20) |
      ((ftoa & 0x00000000000000FF) << 16) | (spider_time & 0x000000000000FFFF);
...
int main(int argc, char** argv) {
  // create a .tpx3 file
  std::ofstream write_file("rollover_test_data.tpx3",
  ...
  // increment counter
  temp =
      packPixelHit(0xF000, 0xFF, 0x3FF, 0x3FFF, 0x9876);  // 25.1662, 1006649343
  write_file.write((char*)&temp, sizeof(unsigned long long));

  temp =
      packPixelHit(0xFF00, 0xFF, 0x3FF, 0x3FFF, 0x9876);  // 26.7391, 1069563903
  write_file.write((char*)&temp, sizeof(unsigned long long));

  temp =
      packPixelHit(0xFFFF, 0xFF, 0x3FF, 0x3FFF, 0x9876);  // 26.8435, 1073741823
  write_file.write((char*)&temp, sizeof(unsigned long long));

  /* --------------------------------------------------------------*/

  temp =
      packPixelHit(0x000F, 0xFF, 0x3FF, 0x3FFF, 0x9876);  // 0.00655357, 262143
  write_file.write((char*)&temp, sizeof(unsigned long long));

  temp =
      packPixelHit(0x00FF, 0xFF, 0x3FF, 0x3FFF, 0x9876);  // 0.104858, 4194303
  write_file.write((char*)&temp, sizeof(unsigned long long));

  temp =
      packPixelHit(0x0FFF, 0xFF, 0x3FF, 0x3FFF, 0x9876);  // 1.67772, 67108863
  write_file.write((char*)&temp, sizeof(unsigned long long));

  temp =
      packPixelHit(0x2FFF, 0xFF, 0x3FF, 0x3FFF, 0x9876);  // 5.03316, 201326591
  write_file.write((char*)&temp, sizeof(unsigned long long));
  ...
  temp =
      packPixelHit(0x3FFF, 0xFF, 0x3FF, 0x3FFF, 0x9876);  // 6.71089, 268435455
  write_file.write((char*)&temp, sizeof(unsigned long long));
  ...
  temp =
      packPixelHit(0x1FFF, 0xFF, 0x3FF, 0x3FFF, 0x9876);  // 3.35544, 134217727
  write_file.write((char*)&temp, sizeof(unsigned long long));
  ...
  // read in .tpx3 file
  std::ifstream read_file("rollover_test_data.tpx3",
                          std::ios::out | std::ios::binary);
  if (!read_file) {
    std::cout << "Cannot open file!" << std::endl;
    return 1;

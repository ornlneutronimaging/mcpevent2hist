...
get_version()
{
  VERSION_MAJOR=$(get_version_part "VERSION_MAJOR")
  VERSION_MINOR=$(get_version_part "VERSION_MINOR")
  VERSION_PATCH=$(get_version_part "VERSION_PATCH")
}
...
commit_version()
{
  commit_version_part "VERSION_PATCH" $1
  commit_version_part "VERSION_MINOR" $2
  commit_version_part "VERSION_MAJOR" $3
...
check_version()
{
  ...
  set -- \
    "$(grep -m 1 "^sophiread (" ${CHANGELOG_FILE}| sed "s/[^0-9.]*//g")" \

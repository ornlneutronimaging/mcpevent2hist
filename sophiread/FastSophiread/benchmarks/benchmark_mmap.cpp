 ...
 * @brief Demonstrate memory-mapped processing converting raw data to events and
 * output.
 * @version 0.2.1
 * @date 2024-01-30
 *
 ...
 * The files are not identical from run-to-run (despite the events being
 * identical) The resulting .h5 file does not load correctly in hdfview (at
 * least I don't know how to see the vectors in it).
 *
 * TODO: There is another program Sophiread that also writes out events.  Make
 * sure that those match up. Better still, use common output writing routines
 * for both programs.
 *
...
void saveEventsToHDF5(const std::string out_file_name,
  ...
  std::transform(events.begin(), events.end(), x.begin(),
                 [](const Neutron& event) { return event.getX(); });
  ...
  std::transform(events.begin(), events.end(), y.begin(),
                 [](const Neutron& event) { return event.getY(); });
  ...
  H5::DataSet tof_ns_dataset =
      group.createDataSet("tof", float_type, dataspace);
  ...
  std::transform(events.cbegin(), events.cend(), tot.begin(),
                 [](const Neutron& event) { return event.getTOT(); });
...
void saveEventsToCSV(const std::string out_file_name,
  ...
  for (auto& event : events) {
    file << event.getX() << "," << event.getY() << "," << event.getTOF_ns()
         << "," << event.getNHits() << "," << event.getTOT() << "\n";
  }
...
void saveEventsToBIN(const std::string out_file_name,
  ...
  // write events
  for (auto& event : events) {
    std::uint16_t x = (std::uint16_t)(
        event.getX() * std::numeric_limits<uint16_t>::max() / 512.0);
    std::uint16_t y = (std::uint16_t)(
        event.getY() * std::numeric_limits<uint16_t>::max() / 512.0);
...
int main(int argc, char* argv[]) {
  if (argc < 2) {
    spdlog::critical("Usage: {} <input file> [<output file> [options]]",
                     argv[0]);
    return 1;
  }
  ...
  timer[TOTAL].begin = timer[RAW_DATA].begin =
      std::chrono::high_resolution_clock::now();
  auto raw_data =
      use_mmap ? mmapTPX3RawToMapInfo(in_tpx3) : readTPX3RawToMapInfo(in_tpx3);
  timer[RAW_DATA].end = std::chrono::high_resolution_clock::now();
  timer[RAW_DATA].accumulated =
      static_cast<double>(std::chrono::duration_cast<std::chrono::microseconds>(
                              timer[RAW_DATA].end - timer[RAW_DATA].begin)
                              .count());
  ...
  while (raw_data_consumed < raw_data.max) {
    ...
    // spdlog::debug("raw_data: {}/{} ({:.2}%)", raw_data_consumed,
    // raw_data.max,
    ...
    std::cerr << "\rraw_data: " << raw_data_consumed << "/" << raw_data.max
              << " ("
              << static_cast<double>(raw_data_consumed) * 100.0 /
                     static_cast<double>(raw_data.max)
              << "%)";
    ...
    timer[BATCHES].end = timer[EVENTS].begin =
        std::chrono::high_resolution_clock::now();
    ...
    if (use_tbb) {
      ...
      tbb::parallel_reduce(tbb::blocked_range<size_t>(0, batches.size()),
                           compute);
      // transfer vectors
    ...
    timer[BATCHES].accumulated += static_cast<double>(
        std::chrono::duration_cast<std::chrono::microseconds>(
            timer[BATCHES].end - timer[BATCHES].begin)
            .count());
    timer[EVENTS].accumulated += static_cast<double>(
        std::chrono::duration_cast<std::chrono::microseconds>(
            timer[EVENTS].end - timer[EVENTS].begin)
            .count());
  }
  ...
  timer[TOTAL].accumulated +=
      static_cast<double>(std::chrono::duration_cast<std::chrono::microseconds>(
                              timer[TOTAL].end - timer[TOTAL].begin)
                              .count());
  ...
  if (!no_output && n_events > 0) {
    ...
    // NB: events is a vector-of-vectors-of-events that we transform into a
    // vector-of-events
    ...
    timer[GATHER].end = timer[OUTPUT].begin =
        std::chrono::high_resolution_clock::now();
    ...
    if (endsWith(out_dat, csv_ext))
      ...
      spdlog::debug("Unhanded extension (.bin, .csv or .h5 are known)");
      no_output = true;
    }
    ...
    timer[GATHER].accumulated += static_cast<double>(
        std::chrono::duration_cast<std::chrono::microseconds>(
            timer[GATHER].end - timer[GATHER].begin)
            .count());
    timer[OUTPUT].accumulated += static_cast<double>(
        std::chrono::duration_cast<std::chrono::microseconds>(
            timer[OUTPUT].end - timer[OUTPUT].begin)
            .count());
    auto speed_gather = n_events / (timer[GATHER].accumulated / 1e6);
    auto speed_output = n_events / (timer[OUTPUT].accumulated / 1e6);
    spdlog::info("{:s} speed: {:<e} events/s", "Gathering Events",
                 speed_gather);
    if (!no_output)
      spdlog::info("{:s} speed: {:<e} events/s", "Writing Output",
                   speed_output);
  }
}

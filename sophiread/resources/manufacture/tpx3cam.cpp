...
int main(int argc, char* argv[]) {
  ...
  if (!myfile) {
    ...
    for (unsigned long long i = 0; i < NumofPacket; i++) {
      ...
      if (HeaderBuffer[0] == 'T' && HeaderBuffer[1] == 'P' &&
          HeaderBuffer[2] == 'X') {
        ...
        for (int j = 0; j < size / 8; j++) {
          ...
          switch (packet) {
            case 0x6:  // TDC timestamp packet header
              ...
              trigtime_fine =
                  (temp & 0x0000000000000E00) | (tmpfine & 0x00000000000001FF);
              time_unit = 25. / 4096;
              trigger_counter = temp >> 44 & 0xFFF;
              TDC_timestamp =
                  coarsetime * 25E-9 + trigtime_fine * time_unit * 1E-9;
              // uncomment below to save TDC timestamps into the txt file
              xy_file << setprecision(15) << TDC_timestamp << endl;
              // cout<< "TDC timestamp: " << setprecision(15) << TDC_timestamp
              // << endl;
              numofTDC = numofTDC + 1;
              break;
            ...
            case 0xb:  // Chip data: ToA and ToT timestamp packet, x, y
              ...
              ************************************************************/
              ...
              xy_file
                  << setprecision(15) << x << "  " << y << "  "
                  << global_timestamp / 1E9 << "  " << TOTns
                  << endl;  // x, y, toa, tot data can be saved into txt data
              cout << "Chip-ToA: " << setprecision(15) << global_timestamp / 1E9
                   << "  ToT: " << TOTns << " x: " << x << " y: " << y << endl;
            ...
            case 0x4:  // the global timestamps.
              ...
              if (((temp >> 56) & 0xF) == 0x4) {
                ...
                // uncomment below to save the global timestamps into the text
                // file;
                xy_file
                    << " Global time: " << setprecision(15)
                    << timemaster * 25e-9
                    << endl;  // global timestamps can be saved into text file
              }
